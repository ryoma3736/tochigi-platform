 ...
 */
export async function getInstagramPosts(limit: number = 9): Promise<InstagramPost[]> {
  // TODO: 本番環境ではInstagram Graph APIから取得
  // const accessToken = process.env.INSTAGRAM_ACCESS_TOKEN
  // const response = await fetch(`https://graph.instagram.com/me/media?fields=...&access_token=${accessToken}`)

  // 現在はモックデータを返す
  return MOCK_INSTAGRAM_POSTS.slice(0, Math.min(limit, MOCK_INSTAGRAM_POSTS.length))
}
